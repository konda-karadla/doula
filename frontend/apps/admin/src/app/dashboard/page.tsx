...
export default function DashboardPage() {
  ...
  const statsCards = [
    ...
    {
      name: 'Lab Results',
      value: (stats?.labResults ?? 0).toLocaleString(),
      change: '+8%',
      changeType: 'positive' as const,
      icon: FileText,
    },
    {
      name: 'Action Plans',
      value: (stats?.actionPlans ?? 0).toLocaleString(),
      change: `${stats?.completionRate ?? 0}%`,
      changeType: 'positive' as const,
      icon: Target,
    },
    {
      name: 'Processing Labs',
      value: (stats?.processingLabs ?? 0).toLocaleString(),
      change: `${stats?.errorLabs ?? 0} errors`,
      changeType: (stats?.errorLabs ?? 0) > 0 ? ('negative' as const) : ('positive' as const),
      icon: Activity,
    },
  ]
